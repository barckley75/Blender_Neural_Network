...
### D. Build the Geometry Nodes tree

Open Blender 5.1. In the **Scripting** workspace, open [build_gn_tree.py](build_gn_tree.py) and click **Run Script**. This writes `neural_network/NeuralNetwork.blend`.

### E. Install the addon

Zip the [neural_network/](neural_network/) folder (with `NeuralNetwork.blend` inside it). Then in Blender: **Edit → Preferences → Add-ons → Install** → pick the zip → enable **"Artificial Neural Network"**.

### F. Add the network and configure it for MNIST

1. **3D Viewport → Add → Neural Network.** A 3D network appears.
...
Rules:
- Number of features must equal **Input Size** (L0).
- Number of classes must be ≤ **Output Size** (last layer).
...
neural_network/          Addon package
  ...
  tree_builder.py        Geometry Nodes tree generator
